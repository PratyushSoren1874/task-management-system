import Todo from "../model/Todo";
import dbConnect from "../utils/dbConnect";
import Link from 'next/link';
import { redirect } from "next/navigation";

export default async function show(){
    dbConnect()
    const todo = await Todo.find();

    // Function to delete todo
    async function deleteTodo(data){
        "use server"
        let id = JSON.parse(data.get("id")?.valueOf());

        await Todo.deleteOne({_id: id});
        redirect('/show');
    }

    return(
        <main className="m-10 space-y-5">
            <h1 className="text-xl font-bold"> Tasks </h1>
            <div>
                <ul className="flex font-bold">
                    <li className="flex-1"> Title </li>
                    <li className="flex-1"> Description </li>
                    <li className="flex-1"> Status </li>
                    <li className="flex-1"> Options </li>
                </ul>
                <hr />
                {todo.map((element)=>{
                    return(
                        <>
                        <ul className="flex" key={element._id}>
                            <li className="flex-1">{element.title}</li>
                            <li className="flex-1">{element.description}</li>
                            <li className="flex-1">{element.status}</li>
                            <li className="flex-1">
                                <div className="flex">
                                <form action={deleteTodo}>
                                <input type="hidden" name="id" id="id" value={JSON.stringify(element._id)}/>
                                <button className="p-2 m-2 bg-red-600 text-white hover:cursor-pointer rounded-xl" type="submit"> Delete </button>
                                </form>
                            
                                <Link href={"/edit/" + element._id}>
                                <button className="p-2 m-2 bg-green-600 text-white hover:cursor-pointer rounded-xl"> Edit </button>
                                </Link>
                                </div>
                            </li>
                        </ul>
                      <hr/>
                    </>
                    );
                })}
            </div>
        </main>
    );
}